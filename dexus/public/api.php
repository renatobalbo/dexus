...
// ============= MODALIDADES =============
    ...
    case 'salvar_modalidade':
        ...
        $sqlDesc = "SELECT COUNT(*) as total FROM CADMOD WHERE MODDES = :descricao";
        $paramsDesc = [':descricao' => $modalidadeData['MODDES']];
        
        if ($exists) {
            // Se for edição, excluir o próprio registro da verificação
            $sqlDesc .= " AND MODCOD <> :modcod";
            $paramsDesc[':modcod'] = $id;
        }
        
        $existsDescResult = fetchOne($sqlDesc, $paramsDesc);
        $existsDesc = $existsDescResult && $existsDescResult['total'] > 0;
        
        if ($existsDesc) {
            $response = [
                'success' => false, 
                'message' => 'Já existe uma modalidade com esta descrição. Por favor, use outra descrição.'
            ];
            break;
        }
