...
if (isset($_GET['codigo']) && $_GET['codigo'] !== '') {
    $whereConditions[] = "CONCOD = :codigo";
    $whereParams[':codigo'] = $_GET['codigo'];
}

if (isset($_GET['nome']) && $_GET['nome'] !== '') {
    $whereConditions[] = "CONNOM LIKE :nome";
    $whereParams[':nome'] = '%' . $_GET['nome'] . '%';
}

if (isset($_GET['atuacao']) && $_GET['atuacao'] !== '') {
    $whereConditions[] = "CONATU LIKE :atuacao";
    $whereParams[':atuacao'] = '%' . $_GET['atuacao'] . '%';
}
...
$paramsWithPagination = $whereParams;
$paramsWithPagination[':offset'] = $offset;
$paramsWithPagination[':limit'] = $itensPorPagina;

// Consulta com paginação para SQL Server
$sql = "SELECT * " . $sqlBase . $whereClause . 
       " ORDER BY CONCOD DESC OFFSET :offset ROWS FETCH NEXT :limit ROWS ONLY";

// Executar a consulta
$consultores = fetchAll($sql, $paramsWithPagination);
...
if ($consultores && count($consultores) > 0) {
    foreach ($consultores as $consultor) {
        ...
        echo "<td>" . formatPhone($consultor['CONTEL'] ?? '') . "</td>";
        echo "<td>{$consultor['CONEMA'] ?? ''}</td>";
        echo "<td>{$consultor['CONATU'] ?? ''}</td>";
        echo "<td>" . ($consultor['CONVAL'] ? formatMoney($consultor['CONVAL']) : '') . "</td>";
        echo "<td>
            <div class='btn-group'>
