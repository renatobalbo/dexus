                ...
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <?php
                            if ($edicao) {
                                // Se for edição, incluir como campo oculto para o POST
                                echo '<input type="hidden" name="CLICOD" value="' . $cliente['CLICOD'] . '">';
                                echo '<input type="text" class="form-control" id="CLICOD" value="' . $cliente['CLICOD'] . '" readonly>';
                            } else {
                                // Se for inclusão, mostrar apenas como informativo (sem name para não enviar no POST)
                                $sqlNextCode = "SELECT ISNULL(MAX(CLICOD), 0) as max_cod FROM CADCLI";
                                $resultNextCode = fetchOne($sqlNextCode);
                                $nextCode = 1; // Código padrão inicial
                                
                                if ($resultNextCode && isset($resultNextCode['max_cod']) && $resultNextCode['max_cod'] > 0) {
                                    $nextCode = (int)$resultNextCode['max_cod'] + 1;
                                }
                                
                                echo '<input type="text" class="form-control" id="CLICOD" value="' . $nextCode . '" readonly>';
                                //echo '<small class="form-text text-muted">Código será gerado automaticamente</small>';
                            }
                            ?>
                        </div>
                    </div>
